...
def np_normalize(v, axis=-1, order=2):
    _l2 = np.linalg.norm(v, ord=order, axis=axis, keepdims=True)
    _l2[_l2 == 0] = 1
    return v / _l2
...
def g_noise21(p: np.array) -> np.array:
    ...
    return 0.5 * np_mix(np_mix(v[0], v[1], f[..., 0], ), np_mix(v[2], v[3], f[..., 1], ), f[..., 1], ) + 0.5
